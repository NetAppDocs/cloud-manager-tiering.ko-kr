...
다음 구성의 ONTAP 클러스터는 Cloud Tiering 라이센스를 사용할 수 있지만 일반적인 단일 노드 및 HA 구성 ONTAP 클러스터와 다른 방식으로 적용되어야 합니다.

* Tiering Mirror의 클러스터 구성(2개의 오브젝트 저장소에 연결된 클러스터)
+
MetroCluster 구성을 사용하는 FabricPool 미러는 현재 지원되지 않습니다
...
. "상위" Cloud Tiering 라이센스는 모든 클러스터가 계층화하는 데 사용되는 용량을 추적하여 라이센스에 충분한 용량이 있는지 확인합니다
. "하위" 계층화 라이센스가 각 클러스터에 자동으로 설치되어 Digital Wallet의 "상위" 라이센스와 통신합니다
...
TIP: 계층화 미러 구성을 위해 데이터가 서로 다른 두 오브젝트 스토리지 위치로 계층화되므로 두 위치로 계층화할 수 있는 충분한 용량의 라이센스를 구입해야 합니다.
